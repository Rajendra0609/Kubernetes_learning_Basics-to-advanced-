...
## 📂 SOLUTIONS INDEX (Added in Augmented Edition)

Solutions to all capstone projects are in `19-capstone-solutions/`:

| Capstone | Solution Location |
|----------|------------------|
| Section 1 — Cluster Health | `19-capstone-solutions/cluster-health/cluster-health-advanced.sh` |
| Section 3 — Canary Release | `03-workload-management/blue-green.yaml` + `canary.yaml` |
| Section 12 — Full E-Commerce App | `19-capstone-solutions/ecommerce-app/ecommerce-full-stack.yaml` |

---

## 🆕 TOPICS ADDED IN AUGMENTED EDITION

| # | Topic | Files Added | Why It Matters |
|---|-------|-------------|----------------|
| 12 | **Namespaces + ResourceQuota + LimitRange** | `01-cluster-setup/namespaces.yaml` | Multi-tenancy foundation; prevents noisy-neighbour resource abuse |
| 13 | **StorageClasses + VolumeSnapshot** | `06-storage/storage-classes.yaml` | gp3/EFS provisioning, cross-region DR from snapshots |
| 14 | **Sealed Secrets / External Secrets / Vault** | `05-config-secrets/sealed-secret-example.yaml` | Never commit plain secrets to Git |
| 15 | **Pod Security Standards (PSS)** | `07-rbac-security/pod-security-standards.yaml` | Replaced deprecated PodSecurityPolicy |
| 16 | **OPA Gatekeeper** | `07-rbac-security/pod-security-standards.yaml` | Policy-as-Code for org-wide guardrails |
| 17 | **Full Helm Chart** | `08-helm/myapp/` | Production chart with multi-env values, HPA, checksums |
| 18 | **KEDA** | `09-autoscaling/keda-scaledobject.yaml` | Scale-to-zero on SQS/Kafka/Prometheus/Cron |
| 19 | **kube-prometheus-stack** | `10-monitoring/kube-prometheus-stack.yaml` | One-helm-chart full monitoring + alerting |
| 20 | **EFK Stack (ECK)** | `11-logging/efk-stack.yaml` | Production-grade log aggregation |
| 21 | **Loki + Promtail** | `11-logging/loki-promtail.yaml` | Lightweight log aggregation, native Grafana integration |
| 22 | **CRD + Operator Pattern** | `14-crd-operators/` | Automate day-2 operations; custom domain APIs |
| 23 | **ArgoCD GitOps** | `15-gitops-cicd/argocd-app.yaml` | Declarative deployments, auto-sync, image updater |
| 24 | **GitHub Actions CI/CD** | `15-gitops-cicd/github-actions-pipeline.yaml` | Full pipeline: test → build → Trivy scan → helm deploy |
| 25 | **Istio Service Mesh** | `16-service-mesh/istio-basics.yaml` | mTLS, traffic splitting, circuit breaker, chaos injection |
| 26 | **Multi-Tenancy** | `17-multitenancy/namespace-isolation.yaml` | Team isolation, per-ns RBAC, NetworkPolicy baseline |
| 27 | **Velero DR + etcd Backup** | `18-disaster-recovery/` | Cluster backup, restore, RTO/RPO planning |
| 28 | **Advanced Ingress** | `04-services-networking/ingress-advanced.yaml` | TLS automation, rate limiting, CORS, security headers |
| 29 | **Dockerfile Best Practices** | `19-capstone-solutions/ecommerce-app/app/Dockerfile` | Multi-stage, non-root, HEALTHCHECK, layer caching |
